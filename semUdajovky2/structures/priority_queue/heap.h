...
namespace structures
{
	...
	void Heap<T>::push(const int priority, const T& data)
	{
		...
		int indexNew = PriorityQueueList<T>::list_->size() - 1;
		int indexParent = getParentIndex(indexNew);

		while (indexParent >= 0 && priority < ((*PriorityQueueList<T>::list_)[indexParent])->getPriority()) {
			DSRoutines::swap((*PriorityQueueList<T>::list_)[indexNew], (*PriorityQueueList<T>::list_)[indexParent]);
			indexNew = indexParent;
			indexParent = getParentIndex(indexNew);
	...
	T Heap<T>::pop()
	{
		...
		if (!PriorityQueueList<T>::list_->isEmpty()) {
			int indexBad = 0;
			int indexSon = getGreaterSonIndex(indexBad);
			int priority = ((*PriorityQueueList<T>::list_)[indexBad])->getPriority();

			while (indexSon != -1 && priority > ((*PriorityQueueList<T>::list_)[indexSon])->getPriority()) {
				DSRoutines::swap((*PriorityQueueList<T>::list_)[indexBad], (*PriorityQueueList<T>::list_)[indexSon]);
				indexBad = indexSon;
				indexSon = getGreaterSonIndex(indexBad);
			}
		}
	...
	inline int Heap<T>::getGreaterSonIndex(const int index)
	{
		...
		if (leftSon >= PriorityQueueList<T>::list_->size()) {
			return -1;
		}
		else {
			if (rightSon >= PriorityQueueList<T>::list_->size()) {
				return leftSon;
			}
			else {
